 ...
 * Uses /api/v1/trackfin/admin/* prefix via centralized apiRequest.
 */
...
/**
 * @param {string} endpoint e.g., '/stats'
 * @param {object} options
 */
const adminFetch = async (endpoint, options = {}) => {
  // apiRequest internally uses resolveUrl which prepends /api/v1
  // We specify the full trackfin/admin namespace here.
  return apiRequest(`/trackfin/admin${endpoint}`, options);
};
...
/** GET /admin/system-health — System environment info */
  ...
  if (USE_MOCK)
    return mockDelay({
      ...
      database: {
        connection: "mysql",
        status: "Healthy",
        db_name: "trackfin_mock",
      },
      server: { os: "Linux", software: "Apache/2.4.52", uptime: "Online" },
...
/** GET /admin/users/:id/transactions — Paginated user transactions */
export const getUserTransactions = (id, params = {}) => {
  if (USE_MOCK) return mockDelay(adminLedgerTransactions);
  const query = new URLSearchParams(params).toString();
  return adminFetch(`/users/${id}/transactions${query ? `?${query}` : ""}`);
};
...
export const getAdminReportStats = () => {
  if (USE_MOCK)
    return mockDelay({
      ...
      exportHistory: adminExportHistory,
    });
...
export const exportUsersCSV = () => {
  exportCSV("trackfin_users.csv", usersData, [
    { key: "id", label: "ID" },
    { key: "name", label: "Name" },
...
export const exportTransactionsCSV = () => {
  exportCSV("trackfin_transactions.csv", adminLedgerTransactions, [
    { key: "id", label: "ID" },
    { key: "date", label: "Date" },
    { key: "user", label: "User" },
...
export const exportAnalyticsSummary = () => {
  exportCSV("trackfin_analytics.csv", adminReportStats, [
